...
src -> components
src -> types.ts 라는 파일을 만들겁니다. 자동차 데이터의 타입을 정의하여 안정성을 높일 겁니다.
...
이제 https://localhost:8080/api/cars 엔드포인트로 GET요청을 전송하면 백엔드에서 자동차를 가져오는 함수를 정의할겁니다.
...
```tsx
  const getCars = async (): Promise<CarResponse[]> => {
    const response = axios.get('http://localhost8080/api/cars');

    return (await response).data._embedded.cars;
  }
```
